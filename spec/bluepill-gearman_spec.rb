...
describe Bluepill::Gearman::SendGearman do
  ...
  let(:notifier) {Bluepill::Gearman::SendGearman.new(process, {:send_gearman => 'remotehost', :host => 'localhost'})}
  ...
  it "should create a default_args instance variable to store default connection parameters" do
    ...
    notifier.instance_variable_get(:@default_args).must_equal({:gearman_server => ['remotehost:4730'], :hostname => 'localhost', :service => 'my_process', :queue => 'check_results'})
  end
  ...
  it "should notify gearman_server of a critical error via send_gearman when a transition to down occurs" do
    ...
    notifier.expects(:send_gearman).with({:send_gearman => 'remotehost', :hostname => 'localhost', :service => 'my_process', :return_code => 2, :status => "Bluepill reported process down at #{Time.now}"})
  ...
  it "should notify gearman_server of a warning error via send_gearman when a transition to unmonitored occurs" do
    ...
    notifier.expects(:send_gearman).with({:send_gearman => 'remotehost', :hostname => 'localhost', :service => 'my_process', :return_code => 1, :status => "Bluepill stopped monitoring at #{Time.now}"})
    notifier.notify(transition)
  end
  ...
  it "should notify gearman_server that everything is ok via send_gearman when a transition to up occurs" do
    ...
    notifier.expects(:send_gearman).with({:send_gearman => 'remotehost', :hostname => 'localhost', :service => 'my_process', :return_code => 0, :status => "Running"})
    notifier.notify(transition)
  end
end
